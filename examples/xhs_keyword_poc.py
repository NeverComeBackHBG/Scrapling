...
def search_note_ids_render(session: Any, keyword: str, max_notes: int = 20, scroll_rounds: int = 4) -> Dict[str, XHSNote]:
...
def run_render_mode(keyword: str, max_notes: int, cookie_header: str = "", scroll_rounds: int = 4) -> List[XHSNote]:
    ...
    with StealthySession(**session_kwargs) as session:
        notes_map = search_note_ids_render(session, keyword=keyword, max_notes=max_notes, scroll_rounds=scroll_rounds)
